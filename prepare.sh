...
IOR_FLAGS=${IOR_EXTRA_CONFIGURE_FLAGS:-""}

# Extra CFLAGS and LDFLAGS for the io500 build (e.g. for custom libraries like DAOS)
IO500_CFLAGS=${IO500_EXTRA_CFLAGS:-""}
IO500_LDFLAGS=${IO500_EXTRA_LDFLAGS:-""}
...
function build_ior {
  ...
  ./configure --prefix="$INSTALL_DIR" $IOR_FLAGS
  cd src
  $MAKE clean
  $MAKE install
  echo "IOR: OK"
...
function build_io500 {
  # Generate local.mk if extra flags are provided
  if [ -n "$IO500_CFLAGS" ] || [ -n "$IO500_LDFLAGS" ]; then
    echo "Generating local.mk for custom CFLAGS/LDFLAGS"
    echo "# Generated by prepare.sh" > local.mk
    if [ -n "$IO500_CFLAGS" ]; then
      echo "CFLAGS += $IO500_CFLAGS" >> local.mk
    fi
    if [ -n "$IO500_LDFLAGS" ]; then
      echo "LDFLAGS += $IO500_LDFLAGS" >> local.mk
    fi
  fi
