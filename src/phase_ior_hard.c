...
void ior_hard_add_params(u_argv_t * argv){
  ...
  u_argv_push(argv, "-e");
  u_argv_push(argv, "-o");
  u_argv_push_printf(argv, "%s/ior_hard/file", opt.datadir);
  u_argv_push(argv, "-O");
  u_argv_push_printf(argv, "stoneWallingStatusFile=%s/ior_hard/stonewall", opt.datadir );
  u_argv_push(argv, "-O");
